...
impl From<JITError> for DSLCompileError {
    fn from(err: JITError) -> Self {
        DSLCompileError::JITError(err.to_string())
    }
}
...
impl JITCompiler {
    ...
    pub fn new() -> Result<Self> {
        ...
        flag_builder
            ...
            .map_err(|e| DSLCompileError::JITError(format!("Failed to set Cranelift flags: {e}")))?;
        flag_builder
            .set("is_pic", "false")
            .map_err(|e| DSLCompileError::JITError(format!("Failed to set Cranelift flags: {e}")))?;
    ...
    pub fn compile_single_var(
        ...
        self.module
            ...
            .map_err(|e| DSLCompileError::JITError(format!("Failed to finalize definitions: {e}")))?;
    ...
    pub fn compile_multi_vars(
        ...
        if var_names.is_empty() {
            return Err(DSLCompileError::JITError(
...
fn generate_ir_for_expr(
    ...
    match expr {
        ...
        ASTRepr::Variable(name) => var_map
            .get(name)
            .copied()
            .ok_or_else(|| DSLCompileError::JITError(format!("Unknown variable: {name}"))),
...
impl JITCompiler {
    pub fn new() -> Result<Self> {
        Err(DSLCompileError::FeatureNotEnabled("cranelift".to_string()))
    }
}
