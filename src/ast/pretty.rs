...
//! using variable names from a `TypedVariableRegistry`.
//!
...
pub fn pretty_ast<T: NumericType>(expr: &ASTRepr<T>, registry: &TypedVariableRegistry) -> String {
    match expr {
...
pub fn pretty_anf<T: NumericType>(expr: &ANFExpr<T>, registry: &TypedVariableRegistry) -> String {
    fn atom<T: NumericType>(a: &ANFAtom<T>, registry: &TypedVariableRegistry) -> String {
        match a {
            ANFAtom::Constant(v) => format!("{v}"),
            ANFAtom::Variable(var_ref) => var_ref.debug_name(registry),
        }
    }
    fn comp<T: NumericType>(c: &ANFComputation<T>, registry: &TypedVariableRegistry) -> String {
        match c {
