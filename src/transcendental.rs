...
fn ln_1plus_approx(x: f64) -> f64 {
    ...
    let num_coeffs = [0.0, 0.9999999985585198, 1.3031632785795166, 0.4385659053064146, 0.03085953976409006];
    
    // Denominator coefficients (constant term first)  
    let den_coeffs = [1.0, 1.8031632248969947, 1.0068149572238094, 0.18320686065538652, 0.0068149572238094085];
...
fn exp_approx(x: f64) -> f64 {
    // Numerator coefficients (constant term first)
    let num_coeffs = [0.9999999999817503, 0.42917796261241653, 0.07164583021340264, 0.004783741506421553];
    
    // Denominator coefficients (constant term first)  
    let den_coeffs = [1.0, -0.5708220372139153, 0.1424678650066201, -0.018939778254233708, 0.001176234397980246];
...
fn sin_approx(x: f64) -> f64 {
    // Numerator coefficients (constant term first)
    let num_coeffs = [0.0, 1.0000000000125895, 0.0, -0.15266388542550388, 0.0, 0.005999540118150915, 0.0, -8.173670882958005e-5];
...
fn cos_approx(x: f64) -> f64 {
    // Numerator coefficients (constant term first)
    let num_coeffs = [1.0000000000849252, -0.04419808517009371, -0.468545034572871, 0.022095248245365844, 0.025958373239365604, -0.0018934016585943506];
