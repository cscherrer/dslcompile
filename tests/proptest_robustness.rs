use mathcompile::anf::{convert_to_anf, ANFExpr, ANFComputation, ANFAtom, VarRef};
...
fn arb_expr_with_config(config: ExprConfig) -> impl Strategy<Value = (DebugExpr, VariableRegistry, Vec<f64>)> {
    ...
    var_strategy.prop_flat_map(move |(var_names, var_values)| {
...
fn arb_expr_recursive(
    ...
    if depth >= config.max_depth || var_indices.is_empty() {
        ...
        arb_expr_recursive(var_indices.clone(), config, config.max_depth).prop_recursive(
            8, // cases per level
            256, // max total cases
            ...
            move |inner| {
                ...
                // Binary operations
                strategies.push((inner.clone(), inner.clone())
                    .prop_map(|(a, b)| ASTEval::add(a, b)).boxed());
                strategies.push((inner.clone(), inner.clone())
                    .prop_map(|(a, b)| ASTEval::sub(a, b)).boxed());
                strategies.push((inner.clone(), inner.clone())
                    .prop_map(|(a, b)| ASTEval::mul(a, b)).boxed());
                ...
                strategies.push((inner.clone(), inner.clone())
                    ...
                    .prop_map(|(a, b)| ASTEval::div(a, b)).boxed());
                ...
                strategies.push((inner.clone(), inner.clone())
                    ...
                    .prop_map(|(a, b)| ASTEval::pow(a, b)).boxed());
                ...
                if config.include_transcendental {
                    ...
                    strategies.push(inner.clone().prop_map(ASTEval::sqrt).boxed());
                    strategies.push(inner.clone().prop_map(ASTEval::exp).boxed());
                    
                    // Natural log with positive argument filtering
                    strategies.push(inner.clone()
                        .prop_filter("positive arguments for ln", |_| true) // Will check during eval
                        .prop_map(ASTEval::ln).boxed());
                }
...
fn evaluate_with_strategy(
    ...
    match strategy {
        ...
        EvalStrategy::ANF => {
            ...
            let var_map: HashMap<usize, f64> = (0..values.len())
        ...
        match (direct_result, anf_result) {
            (Ok(direct), Ok(anf)) => {
                ...
                if !is_numeric_equivalent(direct, anf, 1e-8) {
                    println!("Deep expression precision difference: direct={}, anf={}", direct, anf);
...
mod tests {
    ...
    fn test_manual_failing_case() {
        use mathcompile::final_tagless::{ASTEval, ASTMathExpr, VariableRegistry, DirectEval};
        ...
        if let ANFExpr::Let(var1, comp1, body1) = &anf {
            ...
            let step1_result = match comp1 {
                ANFComputation::Add(ANFAtom::Variable(VarRef::User(0)), ANFAtom::Constant(c)) => 0.0 + c,
            ...
            if let ANFExpr::Let(var2, comp2, body2) = body1.as_ref() {
                ...
                let step2_result = match comp2 {
                    ANFComputation::Add(ANFAtom::Variable(VarRef::User(0)), ANFAtom::Variable(VarRef::User(0))) => 0.0 + 0.0,
        ...
        assert!((direct_result - anf_result).abs() < 1e-10, 
            "Direct: {}, ANF: {}", direct_result, anf_result);
    }
    ...
    fn test_known_equivalent_expressions() {
        ...
        let result1 = evaluate_with_strategy(&expr1, &registry, &values, EvalStrategy::Direct).unwrap();
        let result2 = evaluate_with_strategy(&expr2, &registry, &values, EvalStrategy::Direct).unwrap();
