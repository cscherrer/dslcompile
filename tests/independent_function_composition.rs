...
fn test_independent_function_composition() {
    ...
    let f_result = math_f.eval(&f_expr, &[2.0]); // f(2) = 4 + 4 + 1 = 9
    let g_result = math_g.eval(&g_expr, &[3.0]); // g(3) = 9 + 5 = 14
...
fn test_variable_collision_demonstration() {
    ...
    assert_eq!(math_f.eval(&f_expr, &[5.0]), 10.0); // f(5) = 2*5 = 10
    assert_eq!(math_g.eval(&g_expr, &[7.0]), 21.0); // g(7) = 3*7 = 21
