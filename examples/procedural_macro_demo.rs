...
fn main() {
    ...
    let result4 = optimize_compile_time!(
        var::<0>().ln().add(var::<1>().ln()).exp(),
        [a, b]
    );
    let manual4 = a * b;
    
    println!("  Input: exp(ln(x) + ln(y)) where x = {}, y = {}", a, b);
    ...
    println!("  Input: ln(exp(x)) + y * 1 + 0 * z where x = {}, y = {}, z = {}", x5, y5, z5);
