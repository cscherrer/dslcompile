...
use mathcompile::compile_time::{MathExpr, constant, eval_ast, var};
...
fn main() {
    ...
    let x: f64 = 2.5;
    let y: f64 = 1.5;
    let manual_result = x.sin() + y.cos();
    let macro_result = optimize_compile_time!(var::<0>().sin().add(var::<1>().cos()), [x, y]);
    println!("Manual result: {manual_result:.10}");
    println!("Macro result:  {macro_result:.10}");
    ...
    let macro_result2 = optimize_compile_time!(
        var::<0>()
            .exp()
            .ln()
    ...
    println!("Expected (x + y): {manual_result2:.10}");
    println!("Macro result:     {macro_result2:.10}");
