...
use mathcompile::final_tagless::{
    ASTEval, ASTMathExpr, DirectEval, MathExpr as FinalTaglessMathExpr,
};
...
fn main() -> Result<()> {
    ...
    let manual_ns_per_eval = manual_time.as_nanos() as f64 / f64::from(iterations);

    println!("  Code: x_val");
    println!("  Result: {}", manual_sum / f64::from(iterations));
    println!("  Time: {manual_time:?} ({manual_ns_per_eval:.2} ns/eval)");
    ...
    let macro_ns_per_eval = macro_time.as_nanos() as f64 / f64::from(iterations);

    println!(
        "  Code: optimize_compile_time!(var::<0>().exp().ln().add(constant(0.0).mul(var::<1>())), [x_val, y_val])"
    );
    println!("  Result: {}", macro_sum / f64::from(iterations));
    println!("  Time: {macro_time:?} ({macro_ns_per_eval:.2} ns/eval)");
    ...
    let ct_ns_per_eval = ct_time.as_nanos() as f64 / f64::from(iterations);

    println!("  Code: x.eval(&[x_val, y_val]) // manually optimized from ln(exp(x)) + 0*y");
    println!("  Result: {}", ct_sum / f64::from(iterations));
    println!("  Time: {ct_time:?} ({ct_ns_per_eval:.2} ns/eval)");
    ...
    let ct_egglog_ns_per_eval = ct_egglog_time.as_nanos() as f64 / f64::from(iterations);

    println!("  Code: ct_expr.to_ast() → egglog → optimized.eval_with_vars(&[x_val, y_val])");
    println!("  Result: {}", ct_egglog_sum / f64::from(iterations));
    println!("  Time: {ct_egglog_time:?} ({ct_egglog_ns_per_eval:.2} ns/eval)");
    ...
    let ft_ns_per_eval = ft_time.as_nanos() as f64 / f64::from(iterations);

    println!("  Code: ASTMathExpr::add(...) → egglog → optimized.eval_with_vars(&[x_val, y_val])");
    println!("  Result: {}", ft_sum / f64::from(iterations));
    println!("  Time: {ft_time:?} ({ft_ns_per_eval:.2} ns/eval)");
    ...
    let mb_ns_per_eval = mb_time.as_nanos() as f64 / f64::from(iterations);

    println!(
        "  Code: math.optimize(&expr) → math.eval(&optimized, &[(\"x\", x_val), (\"y\", y_val)])"
    );
    println!("  Result: {}", mb_sum / f64::from(iterations));
    println!("  Time: {mb_time:?} ({mb_ns_per_eval:.2} ns/eval)");
    ...
    let results = [
        (
            "Manual Rust",
            manual_ns_per_eval,
            manual_sum / f64::from(iterations),
        ),
        (
            "Procedural Macro",
            macro_ns_per_eval,
            macro_sum / f64::from(iterations),
        ),
        (
            "Compile-Time Traits",
            ct_ns_per_eval,
            ct_sum / f64::from(iterations),
        ),
        (
            "CT-Traits + Egglog",
            ct_egglog_ns_per_eval,
            ct_egglog_sum / f64::from(iterations),
        ),
        (
            "Final Tagless + Egglog",
            ft_ns_per_eval,
            ft_sum / f64::from(iterations),
        ),
        (
            "Final Tagless Direct",
            ft_direct_ns_per_eval,
            ft_direct_sum / f64::from(iterations),
        ),
        (
            "MathBuilder + Egglog",
            mb_ns_per_eval,
            mb_sum / f64::from(iterations),
        ),
    ];

    println!(
        "{:<25} │ {:>12} │ {:>12} │ {:>8}",
        "System", "ns/eval", "Result", "Speedup"
    );
    println!("─────────────────────────┼──────────────┼──────────────┼─────────");
    ...
    for (i, (name, ns_per_eval)) in optimized_systems.iter().enumerate() {
        ...
        println!("  {emoji} {rank}. {name:<25} ({ns_per_eval:.2} ns/eval)");
    }
