...
fn main() -> Result<()> {
    ...
    println!("ln(10.0/2.0) = ln(5.0) = {:.6}", original_result);
    
    // Expected result using the rule: ln(a) - ln(b)
    let expected = 10.0_f64.ln() - 2.0_f64.ln();
    println!("ln(10.0) - ln(2.0) = {:.6}", expected);
    ...
    #[cfg(feature = "optimization")]
    {
        ...
        println!("Optimization preserved correctness: {}", 
        ...
        if let Ok(mut native_optimizer) = NativeEgglogOptimizer::new() {
            ...
            let const_safe = native_optimizer.is_domain_safe(&pos_const, "ln").unwrap_or(false);
            ...
            println!("Original result: {:.6}", const_expected);
            println!("Optimized result: {:.6}", const_result);
            println!("Native egglog preserved correctness: {}", 
    ...
    println!("ln(-5.0/2.0) = {:.6} (NaN expected due to negative argument)", negative_result);
    
    // Test with zero
    println!("Testing with a = 0.0, b = 2.0:");
    let zero_values = vec![0.0, 2.0];
    let zero_result = DirectEval::eval_with_vars(&ln_div, &zero_values);
    println!("ln(0.0/2.0) = {:.6} (-inf expected)", zero_result);
