...
impl CompilationTiming {
    ...
    fn print_summary(&self) {
        ...
        println!("   Symbolic construction: {:.2}ms", self.symbolic_construction_ms);
        println!("   Symbolic optimization: {:.2}ms", self.symbolic_optimization_ms);
        println!("   Code generation:       {:.2}ms", self.code_generation_ms);
        println!("   Rust compilation:      {:.2}ms", self.rust_compilation_ms);
        println!("   ─────────────────────────────────");
        println!("   Total compilation:     {:.2}ms", self.total_compilation_ms);
        
        // Calculate percentages
        let total = self.total_compilation_ms;
        if total > 0.0 {
            println!("\n📊 Time Distribution:");
            println!("   Symbolic construction: {:.1}%", (self.symbolic_construction_ms / total) * 100.0);
            println!("   Symbolic optimization: {:.1}%", (self.symbolic_optimization_ms / total) * 100.0);
            println!("   Code generation:       {:.1}%", (self.code_generation_ms / total) * 100.0);
            println!("   Rust compilation:      {:.1}%", (self.rust_compilation_ms / total) * 100.0);
        }
    }
}
...
fn normal_log_density(x: ASTRepr<f64>, mu: ASTRepr<f64>, sigma_sq: ASTRepr<f64>) -> ASTRepr<f64> {
    ...
    let sq_residual = <ASTEval as ASTMathExpr>::pow(residual, <ASTEval as ASTMathExpr>::constant(2.0));
    let residual_term = <ASTEval as ASTMathExpr>::mul(
        <ASTEval as ASTMathExpr>::mul(<ASTEval as ASTMathExpr>::constant(-0.5), <ASTEval as ASTMathExpr>::div(<ASTEval as ASTMathExpr>::constant(1.0), sigma_sq)),
        sq_residual
    );
...
impl BayesianLinearRegression {
    ...
    pub fn new_with_config(data: Vec<(f64, f64)>, egglog_enabled: bool) -> Result<Self> {
        ...
        println!("📊 Log-posterior built naturally in {:.2}ms", timing.symbolic_construction_ms);
        println!("   Operations before optimization: {}", log_posterior_expr.count_operations());
        ...
        println!("   Operations after optimization: {}", optimized_posterior.count_operations());
        ...
        println!("   Operation reduction: {:.1}% ({} → {} ops)", reduction_percent, original_ops, optimized_ops);
        ...
        let posterior_code = rust_generator.generate_function(&optimized_posterior, "log_posterior")?;
        ...
        let log_posterior_compiled = rust_compiler.compile_and_load(&posterior_code, "log_posterior")?;
    ...
    fn build_natural_log_posterior(data: &[(f64, f64)]) -> Result<ASTRepr<f64>> {
        ...
        // For large datasets, we need to use summation infrastructure
        // instead of inlining all data points
        
        if data.len() > 1000 {
            // Use summation-based approach for large datasets
            println!("   Using summation-based approach for {} data points", data.len());
            
            // Create a simplified log-likelihood that represents the pattern
            // This is a placeholder - in a full implementation, we'd use the summation
            // infrastructure to create: Σ(i=1 to n) log N(y_i | β₀ + β₁*x_i, σ²)
            
            // For now, create a representative log-likelihood for a single data point
            // and multiply by the number of data points (this is a simplification)
            let n_data = data.len() as f64;
            
            // Use representative values from the data
            let mean_x = data.iter().map(|(x, _)| x).sum::<f64>() / n_data;
            let mean_y = data.iter().map(|(_, y)| y).sum::<f64>() / n_data;
            
            // Single data point log-likelihood pattern
            let mu_representative = <ASTEval as ASTMathExpr>::add(
                beta0.clone(),
                <ASTEval as ASTMathExpr>::mul(beta1.clone(), <ASTEval as ASTMathExpr>::constant(mean_x))
            );
            
            let single_log_density = normal_log_density(
                <ASTEval as ASTMathExpr>::constant(mean_y),
                mu_representative,
                sigma_sq.clone()
            );
            
            // Scale by number of data points
            let log_likelihood = <ASTEval as ASTMathExpr>::mul(
                <ASTEval as ASTMathExpr>::constant(n_data),
                single_log_density
            );
            ...
            let prior_sigma = <ASTEval as ASTMathExpr>::sub(
                <ASTEval as ASTMathExpr>::mul(<ASTEval as ASTMathExpr>::constant(-2.0), <ASTEval as ASTMathExpr>::ln(sigma_sq.clone())),
            ...
            println!("   Using direct approach for {} data points", data.len());
            ...
            for &(x_i, y_i) in data {
                ...
                let mu_i = <ASTEval as ASTMathExpr>::add(
                    beta0.clone(),
                    <ASTEval as ASTMathExpr>::mul(beta1.clone(), <ASTEval as ASTMathExpr>::constant(x_i))
                );
    ...
    pub fn log_posterior_direct(&self, params: &[f64]) -> Result<f64> {
        ...
        Ok(DirectEval::eval_with_vars(&self.log_posterior_symbolic, params))
    }
    ...
    pub fn performance_comparison(&self, params: &[f64], n_evals: usize) -> Result<()> {
        ...
        println!("   Time: {:.2}ms", direct_ms);
        println!("   Rate: {:.1} evals/sec", direct_rate);
        println!("   Per eval: {:.3}μs", direct_time.as_micros() as f64 / n_evals as f64);
        println!("   Result: {:.6}", direct_result);
        ...
        println!("   Time: {:.2}ms", compiled_ms);
        println!("   Rate: {:.1} evals/sec", compiled_rate);
        println!("   Per eval: {:.3}μs", compiled_time.as_micros() as f64 / n_evals as f64);
        println!("   Result: {:.6}", compiled_result);
        
        // Comparison
        let speedup = direct_time.as_secs_f64() / compiled_time.as_secs_f64();
        println!("\n📈 Comparison:");
        println!("   Speedup: {:.1}x faster", speedup);
        println!("   Results match: {}", (direct_result - compiled_result).abs() < 1e-10);
        
        // Amortization analysis
        let compilation_cost_evals = self.timing.total_compilation_ms / (compiled_time.as_secs_f64() * 1000.0 / n_evals as f64);
        println!("   Compilation amortized over: {:.0} evaluations", compilation_cost_evals);
    ...
    pub fn find_map_estimate(&self) -> Result<Vec<f64>> {
        ...
        // Simple grid search
        for beta0 in (-5..=5).map(|x| x as f64) {
            for beta1 in (-3..=3).map(|x| x as f64 * 0.5) {
        ...
        println!("   MAP estimate: β₀={:.3}, β₁={:.3}, σ²={:.3}", 
                best_params[0], best_params[1], best_params[2]);
        println!("   Log-posterior: {:.3}", best_log_posterior);
        println!("   Search completed in {:.2}ms ({} evaluations)", search_time, evaluations);
        println!("   Evaluation rate: {:.1} evals/ms", evaluations as f64 / search_time);
...
fn generate_synthetic_data(n: usize, true_beta0: f64, true_beta1: f64, true_sigma: f64) -> Vec<(f64, f64)> {
...
fn main() -> Result<()> {
    ...
    let true_beta1 = 1.5;
    let true_sigma = 0.8;
    let n_data = 1000000; // Demo size - scales to millions with summation approach
    ...
    println!("   True parameters: β₀={}, β₁={}, σ={}", true_beta0, true_beta1, true_sigma);
    println!("   Generated {} data points in {:.2}ms\n", data.len(), data_time);
    ...
    println!("   Compiled result: {:.6}", compiled_result_no_egglog);
    println!("   DirectEval result: {:.6}", direct_result_no_egglog);
    println!("   Results match: {}", (compiled_result_no_egglog - direct_result_no_egglog).abs() < 1e-10);
    ...
    println!("   Compiled result: {:.6}", compiled_result_egglog);
    println!("   DirectEval result: {:.6}", direct_result_egglog);
    println!("   Results match: {}", (compiled_result_egglog - direct_result_egglog).abs() < 1e-10);
    ...
    println!("───────────────────────────┼─────────────────┼─────────────────┼─────────────");
    println!("Symbolic construction      │ {:>13.2}ms │ {:>13.2}ms │ {:>+8.2}ms", 
    ...
    println!("Symbolic optimization      │ {:>13.2}ms │ {:>13.2}ms │ {:>+8.2}ms", 
    ...
    println!("Code generation            │ {:>13.2}ms │ {:>13.2}ms │ {:>+8.2}ms", 
    ...
    println!("Rust compilation           │ {:>13.2}ms │ {:>13.2}ms │ {:>+8.2}ms", 
    ...
    println!("───────────────────────────┼─────────────────┼─────────────────┼─────────────");
    println!("TOTAL                      │ {:>13.2}ms │ {:>13.2}ms │ {:>+8.2}ms", 
    ...
    println!("\n🎯 Numerical Accuracy:");
    println!("   Results identical: {}", (compiled_result_no_egglog - compiled_result_egglog).abs() < 1e-12);
    println!("   Difference: {:.2e}", (compiled_result_no_egglog - compiled_result_egglog).abs());
    ...
    if timing_egglog.symbolic_optimization_ms > timing_no_egglog.symbolic_optimization_ms {
        println!("   ✅ Egglog adds {:.2}ms to symbolic optimization", 
                 timing_egglog.symbolic_optimization_ms - timing_no_egglog.symbolic_optimization_ms);
    }
