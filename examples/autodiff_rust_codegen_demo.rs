...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let expr = JITEval::add(
        JITEval::add(
            JITEval::add(
                ...
                JITEval::mul(JITEval::constant(-3.0), JITEval::pow(JITEval::var("x"), JITEval::constant(2.0))),
            ),
    ...
    for &x_val in &test_points {
        ...
        println!("{:.1}\t\t{:.3}\t\t{:.3}\t\t{:.3}", 
                 x_val, value, derivative, expected_derivative);
    ...
    let multi_var_expr = JITEval::add(
        JITEval::add(
            JITEval::add(
                JITEval::mul(JITEval::pow(JITEval::var("x"), JITEval::constant(2.0)), JITEval::var("y")),
                JITEval::mul(JITEval::var("x"), JITEval::pow(JITEval::var("y"), JITEval::constant(2.0))),
            ),
            JITEval::mul(JITEval::constant(-2.0), JITEval::mul(JITEval::var("x"), JITEval::var("y"))),
        ),
        JITEval::constant(1.0),
    );
    ...
    let expected_dx = 2.0 * test_point[0] * test_point[1] + test_point[1] * test_point[1] - 2.0 * test_point[1];
    let expected_dy = test_point[0] * test_point[0] + 2.0 * test_point[0] * test_point[1] - 2.0 * test_point[0];
    
    println!("Expected ∂g/∂x = {:.3}", expected_dx);
    println!("Expected ∂g/∂y = {:.3}\n", expected_dy);
    ...
    for i in 0..8 {
        ...
        if (x_new - x).abs() < 1e-12 {
            println!("Converged to root: x = {:.12}", x_new);
            break;
        }
