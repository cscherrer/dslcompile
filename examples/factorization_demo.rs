...
use mathcompile::compile_time::{self, MathExpr as CompileTimeMathExpr, *};
...
fn main() {
    ...
    println!("Test: x={}, y={} → {:.1}", test_values[0], test_values[1], expected);
    println!("✅ Discovery successful: {}", (optimized_result - expected).abs() < 1e-10);
    println!();
    ...
    let expected = test_values_multi[0] + test_values_multi[1] + test_values_multi[2] 
                  + test_values_multi[3] - test_values_multi[4];
    
    println!("Expression: ln(exp(x) * exp(y) * exp(z)) + ln(exp(a)) - ln(exp(b))");
    println!("Discovered: x + y + z + a - b");
    println!("Test: x=1, y=2, z=3, a=4, b=1.5 → {:.1}", expected);
    println!("✅ Discovery successful: {}", (complex_result - expected).abs() < 1e-10);
    println!();
    ...
    if simple_time < complex_time {
        ...
        println!("Optimization speedup: {:.1}x faster", speedup);
    } else {
