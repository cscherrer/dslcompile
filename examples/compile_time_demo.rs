...
fn main() {
    ...
    println!("Values: x={}, y={}, z={}", test_values[0], test_values[1], test_values[2]);
    println!("Result: {}", result1);
    println!("Expected: {}", (test_values[0] + test_values[1]) * test_values[2]);
    println!("✅ Matches: {}", result1 == (test_values[0] + test_values[1]) * test_values[2]);
    println!();
    ...
    println!("Result: {:.6}", result2);
    println!("Expected: {:.6}", expected2);
    ...
    println!("Expected (x): {:.10}", test_value);
    println!("✅ Optimization preserves correctness: {}", (optimized_result - test_value).abs() < 1e-10);
    ...
    let complex_expr = x.clone().exp().mul(y.clone().exp()).ln().add(z.clone().exp().ln());
    ...
    println!("Values: x={}, y={}, z={}", test_values3[0], test_values3[1], test_values3[2]);
    println!("Complex result: {:.6}", complex_result);
    println!("Expected simple: {:.6}", expected_simple);
    println!("✅ Mathematical equivalence: {}", (complex_result - expected_simple).abs() < 1e-10);
    println!();
    ...
    fn quadratic_expr() -> impl MathExpr {
        ...
        // ax² + bx + c
        a.clone().mul(x.clone().mul(x.clone())).add(b.clone().mul(x.clone())).add(c.clone())
    }
    ...
    fn exponential_decay_expr() -> impl MathExpr {
        ...
        initial.clone().mul(rate.clone().neg().mul(time.clone()).exp())
    }
    ...
    if simple_time < complex_time {
        ...
        println!("🚀 Simple form is {:.2}x faster!", speedup);
    } else {
    ...
    println!("✅ Results match: {}", (complex_result - simple_result).abs() < 1e-10);
    println!();
