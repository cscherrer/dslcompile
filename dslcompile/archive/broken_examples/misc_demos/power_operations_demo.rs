...
fn main() -> Result<()> {
    ...
    let result = math.eval_old(&expr, &[2.0]); // Variable(0) = 2.0
    println!("2^8 = {result}");
    ...
    let sqrt_result = math.eval_old(&sqrt_expr, &[0.0, 4.0]); // Variable(1) = 4.0
    ...
    let var_result = math.eval_old(&var_power, &[0.0, 0.0, 3.0, 2.0]); // Variable(2) = 3.0, Variable(3) = 2.0
