...
pub trait ASTVisitor<T: Scalar + Clone> {
    ...
    fn visit(&mut self, expr: &ASTRepr<T>) -> Result<Self::Output, Self::Error> {
        ...
        while let Some(work_item) = stack.pop() {
            match work_item {
                ...
                VisitorWorkItem::VisitCollection(collection) => {
                    match &collection {
                        Collection::Empty => {
    ...
    #[deprecated(note = "This method now uses heap-allocated stack internally. The API is preserved for compatibility.")]
    ...
    #[deprecated(note = "This method now uses heap-allocated stack internally. The API is preserved for compatibility.")]
    fn visit_let(&mut self, binding_id: usize, expr: &ASTRepr<T>, body: &ASTRepr<T>) -> Result<Self::Output, Self::Error> {
