...
pub struct RuleConfig {
    ...
    /// Specific rule files to load (in addition to categories)
...
impl RuleLoader {
    ...
    pub fn load_rules(&self) -> Result<String> {
        ...
        // Always load core datatypes first (unless loading custom files only)
        if !self.config.rule_files.is_empty() && self.config.categories.is_empty() {
            // Loading only custom files - ensure core datatypes is included
        ...
        for rule_file in &self.config.rule_files {
            ...
            let content = self.load_custom_rule_file(rule_file)?;
            program.push_str(&content);
            program.push('\n');
        }
    ...
    /// Load a custom rule file by name
    fn load_custom_rule_file(&self, filename: &str) -> Result<String> {
        ...
        for search_path in &self.config.search_paths {
            ...
            if file_path.exists() {
                return fs::read_to_string(&file_path).map_err(|e| {
                    DSLCompileError::Generic(format!(
                        "Failed to load custom rule file {}: {}",
                        file_path.display(),
                        e
                    ))
                });
        ...
        // Try the default rules directory
        ...
        fs::read_to_string(&file_path).map_err(|e| {
            DSLCompileError::Generic(format!(
                "Failed to load custom rule file {} (searched in: {:?}, {}): {}",
                filename,
