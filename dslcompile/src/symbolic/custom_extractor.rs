...
impl SummationCouplingAnalyzer {
    ...
    pub fn calculate_coupling_cost(&mut self, expr: &ASTRepr<f64>) -> Result<f64> {
        ...
        let cost = match pattern {
            CouplingPattern::Decoupled { operation_count, .. } => {
                1.0 + (operation_count as f64) * 0.1
            },
            CouplingPattern::Coupled { cost_multiplier, operation_count, .. } => {
                cost_multiplier + (operation_count as f64) * 10.0
            },
            CouplingPattern::Expandable { cost_reduction, .. } => {
                10.0 * (1.0 - cost_reduction)
            },
        };
    ...
    pub fn get_coupling_report(&self) -> String {
        ...
        report.push_str(&format!("Cached {} cost calculations\n", self.cost_cache.len()));
    ...
    fn collect_variables(&self, expr: &ASTRepr<f64>, variables: &mut std::collections::HashSet<usize>) {
        match expr {
    ...
    fn count_operations(&self, expr: &ASTRepr<f64>) -> usize {
        match expr {
            ...
            ASTRepr::Add(left, right) | ASTRepr::Sub(left, right) | 
            ASTRepr::Mul(left, right) | ASTRepr::Div(left, right) | 
            ASTRepr::Pow(left, right) => 1 + self.count_operations(left) + self.count_operations(right),
            ASTRepr::Neg(inner) | ASTRepr::Ln(inner) | ASTRepr::Exp(inner) | 
            ASTRepr::Sin(inner) | ASTRepr::Cos(inner) | ASTRepr::Sqrt(inner) => 1 + self.count_operations(inner),
        }
    }
}
...
mod tests {
    ...
    fn test_coupling_pattern_analysis() {
        ...
        match pattern {
            CouplingPattern::Coupled { cost_multiplier, .. } => {
                assert!(cost_multiplier > 100.0, "Power operations with multiple variables should have high coupling cost");
            },
    ...
    fn test_cost_calculation() {
        ...
        assert!(cost < 10.0, "Simple addition should have low coupling cost, got {}", cost);
    }
