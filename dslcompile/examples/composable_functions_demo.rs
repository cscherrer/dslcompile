...
fn main() -> Result<()> {
    ...
    println!("✅ Created shared parameters: μ={}, σ={}", mu.var_id(), sigma.var_id());
    ...
    let mixture_normal = create_mixture_normal(&mut ctx, &x_mix, &mu, &sigma, &mu2, &sigma2, &weight);
    ...
    #[cfg(feature = "optimization")]
    {
        ...
        let single_reduction = calculate_reduction(single_ast.count_operations(), optimized_single.count_operations());
        let iid_reduction = calculate_reduction(iid_ast.count_operations(), optimized_iid.count_operations());
        let mixture_reduction = calculate_reduction(mixture_ast.count_operations(), optimized_mixture.count_operations());
        
        println!("   Single Normal:");
        println!("     • Before: {} operations", single_ast.count_operations());
        println!("     • After: {} operations", optimized_single.count_operations());
        println!("     • Result: {}", single_reduction);
        
        println!("   IID Normal (function composition):");
        println!("     • Before: {} operations", iid_ast.count_operations());
        println!("     • After: {} operations", optimized_iid.count_operations());
        println!("     • Result: {}", iid_reduction);
        
        println!("   Mixture Normal (complex composition):");
        println!("     • Before: {} operations", mixture_ast.count_operations());
        println!("     • After: {} operations", optimized_mixture.count_operations());
        ...
        println!("Single normal log-density(x=1, μ=0, σ=1): {:.6}", test_result);
        ...
        println!("   • Before optimization: {} operations", advanced_ast.count_operations());
        println!("   • After optimization: {} operations", optimized_advanced.count_operations());
        println!("   • Reduction: {}", calculate_reduction(advanced_ast.count_operations(), optimized_advanced.count_operations()));
    ...
    #[cfg(not(feature = "optimization"))]
    {
        ...
        println!("   Run with: cargo run --features optimization --example composable_functions_demo");
    }
...
fn calculate_reduction(before: usize, after: usize) -> String {
    if before > after {
        ...
        format!("Optimization added {} operations (may improve numerical stability)", after - before)
    } else {
...
fn compute_depth<T>(ast: &ASTRepr<T>) -> usize {
    match ast {
        ...
        ASTRepr::Let(_, expr, body) => {
            1 + std::cmp::max(compute_depth(expr), compute_depth(body))
        }
        ASTRepr::Neg(inner) | ASTRepr::Ln(inner) | ASTRepr::Exp(inner) | 
        ASTRepr::Sin(inner) | ASTRepr::Cos(inner) | ASTRepr::Sqrt(inner) => {
