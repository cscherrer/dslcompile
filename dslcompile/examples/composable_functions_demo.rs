...
fn compute_depth<T>(ast: &ASTRepr<T>) -> usize {
    match ast {
        ...
        ASTRepr::Add(operands) => {
            let mut max_depth = 0;
            for operand in operands {
                let depth = compute_depth(operand);
                if depth > max_depth {
                    max_depth = depth;
                }
            }
        ...
        ASTRepr::Let(_, expr, body) => {
            ...
            std::cmp::max(expr_depth, body_depth) + 1
        }
        ASTRepr::Neg(inner)
        | ASTRepr::Ln(inner)
        | ASTRepr::Exp(inner)
        | ASTRepr::Sin(inner)
        | ASTRepr::Cos(inner)
        | ASTRepr::Sqrt(inner) => compute_depth(inner) + 1,
        ASTRepr::Lambda(lambda) => compute_depth(&lambda.body) + 1,
        ASTRepr::Sum(_) => 2,
    }
}
