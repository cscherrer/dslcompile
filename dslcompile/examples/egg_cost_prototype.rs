...
mod egg_prototype {
    ...
    impl<'a> CostFunction<MathLang> for SummationCostFunction<'a> {
            ...
            C: FnMut(Id) -> Self::Cost,
        {
            match enode {
                ...
                MathLang::Add([a, b]) => costs(*a) + costs(*b) + self.operation_costs["add"],
                MathLang::Mul([a, b]) => costs(*a) + costs(*b) + self.operation_costs["mul"],
                MathLang::Sub([a, b]) => {
                    costs(*a) + costs(*b) + self.operation_costs["add"] // Sub = Add + Neg
                }
                MathLang::Div([a, b]) => costs(*a) + costs(*b) + self.operation_costs["div"],
                MathLang::Pow([a, b]) => {
                ...
                MathLang::Range([start, end]) => costs(*start) + costs(*end) + 5.0,
                MathLang::Map([lambda, collection]) => {
    ...
    fn is_independent(
        expr: &str,
        var: &str,
    ) -> impl Fn(&mut EGraph<MathLang, DependencyAnalysis>, Id, &Subst) -> bool {
    ...
    pub fn demonstrate_custom_costs() {
        ...
        println!(
            "   Completed {} iterations of rewriting",
