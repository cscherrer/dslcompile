...
use dslcompile::{
    ast::{ASTRepr, ExprArena, arena_to_ast, ast_to_arena},
    prelude::*,
};
...
fn main() {
    ...
    println!(
        "Arena nodes: {} (optimized through structural sharing)",
        arena.len()
    );
    ...
    println!(
        ...
        converted_back.eval_with_vars(&[3.0])
    ...
    println!(
        "Arena nodes for complex expression: {}",
        complex_arena.len()
    );
    ...
    let arena_result = arena_to_ast(_complex_arena_expr, &complex_arena)
        .unwrap()
        .eval_with_vars(&test_values);
