...
fn main() {
    ...
    println!(
        "Bounds-checked: vars.get(0).copied().unwrap_or(0.0) + vars.get(1).copied().unwrap_or(0.0)"
    );
    println!(
        "Time: {:?} ({:.2} ns/op)",
        bounds_check_time,
        bounds_check_time.as_nanos() as f64 / f64::from(iterations)
    );
    println!(
        "Overhead vs Rust: {:.1}x",
        bounds_check_time.as_nanos() as f64 / rust_time.as_nanos() as f64
    );
    println!(
        "Overhead vs Array: {:.1}x",
        bounds_check_time.as_nanos() as f64 / array_time.as_nanos() as f64
    );
    println!();
    ...
    println!(
        "Time: {:?} ({:.2} ns/op)",
        function_time,
        function_time.as_nanos() as f64 / f64::from(iterations)
    );
    println!(
        "Overhead vs Rust: {:.1}x",
        function_time.as_nanos() as f64 / rust_time.as_nanos() as f64
    );
    println!();
    ...
    println!(
        "Time: {:?} ({:.2} ns/op)",
        method_time,
        method_time.as_nanos() as f64 / f64::from(iterations)
    );
    println!(
        "Overhead vs Rust: {:.1}x",
        method_time.as_nanos() as f64 / rust_time.as_nanos() as f64
    );
    println!();
    ...
    println!(
        "Time: {:?} ({:.2} ns/op)",
        trait_object_time,
        trait_object_time.as_nanos() as f64 / f64::from(iterations)
    );
    println!(
        "Overhead vs Rust: {:.1}x",
        trait_object_time.as_nanos() as f64 / rust_time.as_nanos() as f64
    );
    println!();
    ...
    println!(
        "Time: {:?} ({:.2} ns/op)",
        compile_time_time,
        compile_time_time.as_nanos() as f64 / f64::from(iterations)
    );
    println!(
        "Overhead vs Rust: {:.1}x",
        compile_time_time.as_nanos() as f64 / rust_time.as_nanos() as f64
    );
    println!();
    ...
    println!(
        "Time: {:?} ({:.2} ns/op)",
        manual_time,
        manual_time.as_nanos() as f64 / f64::from(iterations)
    );
    println!(
        "Overhead vs Rust: {:.1}x",
        manual_time.as_nanos() as f64 / rust_time.as_nanos() as f64
    );
    println!();
    ...
    println!(
        "1. Pure Rust:           {:>8.2} ns/op (baseline)",
        rust_time.as_nanos() as f64 / f64::from(iterations)
    );
    println!(
        "2. Array access:        {:>8.2} ns/op ({:.1}x)",
        array_time.as_nanos() as f64 / f64::from(iterations),
        array_time.as_nanos() as f64 / rust_time.as_nanos() as f64
    );
    println!(
        "3. Bounds checking:     {:>8.2} ns/op ({:.1}x)",
        bounds_check_time.as_nanos() as f64 / f64::from(iterations),
        bounds_check_time.as_nanos() as f64 / rust_time.as_nanos() as f64
    );
    println!(
        "4. Function calls:      {:>8.2} ns/op ({:.1}x)",
        function_time.as_nanos() as f64 / f64::from(iterations),
        function_time.as_nanos() as f64 / rust_time.as_nanos() as f64
    );
    println!(
        "5. Method calls:        {:>8.2} ns/op ({:.1}x)",
        method_time.as_nanos() as f64 / f64::from(iterations),
        method_time.as_nanos() as f64 / rust_time.as_nanos() as f64
    );
    println!(
        "6. Trait objects:       {:>8.2} ns/op ({:.1}x)",
        trait_object_time.as_nanos() as f64 / f64::from(iterations),
        trait_object_time.as_nanos() as f64 / rust_time.as_nanos() as f64
    );
    println!(
        "7. Compile-time system: {:>8.2} ns/op ({:.1}x)",
        compile_time_time.as_nanos() as f64 / f64::from(iterations),
        compile_time_time.as_nanos() as f64 / rust_time.as_nanos() as f64
    );
    println!(
        "8. Manual Add:          {:>8.2} ns/op ({:.1}x)",
        manual_time.as_nanos() as f64 / f64::from(iterations),
        manual_time.as_nanos() as f64 / rust_time.as_nanos() as f64
    );
    println!();
    ...
    if bounds_overhead > 2.0 {
        println!("• Bounds checking adds significant overhead ({bounds_overhead:.1}x)");
    ...
    if trait_overhead > 2.0 {
        println!("• Trait objects add virtual dispatch overhead ({trait_overhead:.1}x)");
    }

    let system_vs_manual = compile_time_time.as_nanos() as f64 / manual_time.as_nanos() as f64;
    if system_vs_manual > 1.5 {
        println!(
            "• The compile-time system has additional overhead vs manual implementation ({system_vs_manual:.1}x)"
        );
    } else {
