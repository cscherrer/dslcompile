...
mod egg_demo {
    use egg::{
        ...
        LanguageChildren, Rewrite, Runner, Symbol, define_language, rewrite as rw,
    };
    ...
    impl Analysis<SimpleMath> for DepAnalysis {
        ...
        fn make(_egraph: &mut EGraph<SimpleMath, Self>, enode: &SimpleMath) -> Self::Data {
    ...
    impl CostFunction<SimpleMath> for SummationCostFunction {
            ...
            C: FnMut(Id) -> Self::Cost,
        {
            match enode {
                ...
                SimpleMath::Add([a, b]) => costs(*a) + costs(*b) + self.operation_costs["add"],
                SimpleMath::Mul([a, b]) => costs(*a) + costs(*b) + self.operation_costs["mul"],
                SimpleMath::Pow([a, b]) => {
                ...
                // Unary operations
                SimpleMath::Ln([a]) => costs(*a) + self.operation_costs["ln"],
                SimpleMath::Exp([a]) => costs(*a) + self.operation_costs["exp"],
                ...
                SimpleMath::Sum([collection]) => {
                    ...
                    let mock_dependencies =
                        std::collections::BTreeSet::from([Symbol::from("x"), Symbol::from("y")]);
                    ...
                    let iteration_cost =
                        (collection_size as f64) * inner_cost * coupling_multiplier;

                    let total_cost = base_cost + iteration_cost;

                    println!(
                        "💰 NON-ADDITIVE Summation cost: base={base_cost:.1}, size={collection_size}, inner={inner_cost:.1}, coupling={coupling_multiplier:.1}x → total={total_cost:.1}"
                    );
    ...
    pub fn demonstrate_non_additive_costs() {
        ...
        println!(
            "   Rewriting completed in {} iterations",
        ...
        println!("   Cost: {ast_cost:.1} (additive)");
        ...
        println!(
            "   Standard cost functions are additive: cost(parent) = sum(child_costs) + operation_cost"
        );
        println!(
            "   Our summation cost is multiplicative: cost(sum) = base + (size × inner_complexity × coupling)"
        );
    ...
    fn demonstrate_cost_scenarios(egraph: &EGraph<SimpleMath, DepAnalysis>) {
        ...
        println!("   Simple addition cost: {simple_cost:.1}");
        println!(
            "   Sum of simple addition: {:.1} ({}x multiplier)",
            sum_simple_cost,
            sum_simple_cost / simple_cost
        );
        ...
        println!("   Complex expression cost: {complex_cost:.1}");
        println!(
            "   Sum of complex expression: {:.1} ({}x multiplier)",
            sum_complex_cost,
            sum_complex_cost / complex_cost
        );
