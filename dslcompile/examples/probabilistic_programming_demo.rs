...
fn test_gaussian_log_density_optimization(n: usize) -> Result<()> {
    ...
    let data: Vec<f64> = (0..n).map(|i| 2.0 + 1.5 * (i as f64 / n as f64 - 0.5)).collect();
    let mu = 2.0;
    let sigma: f64 = 1.5;
    ...
    let log_density_result = math.sum(&data, |x: TypedBuilderExpr<f64>| -> TypedBuilderExpr<f64> {
        ...
        let log_normalization = math.constant(0.5) * two_pi.clone().ln() + sigma_param.clone().ln(); // 0.5*log(2π) + log(σ)
    ...
    println!("  ✅ Runtime symbolic result: {:.6}", symbolic_result);
    ...
    println!("  📈 Eval speedup: {:.2}x", eval_speedup);
    println!("  📊 Total speedup: {:.2}x", total_speedup);
    println!("  🎯 Accuracy: {:.2e}", accuracy);
    
    if eval_speedup > 10.0 {
        println!("  🚀 SYMBOLIC EVALUATION CRUSHING IT! ({}x faster)", eval_speedup as i32);
    } else if eval_speedup > 1.5 {
        println!("  ⚡ Symbolic evaluation winning!");
    } else if total_speedup > 0.8 {
        println!("  ⚖️  Competitive overall (eval: {:.1}x, total: {:.1}x)", eval_speedup, total_speedup);
    } else {
        println!("  📝 Build cost dominates (eval: {:.1}x faster, but total: {:.2}x)", eval_speedup, total_speedup);
    }
...
mod tests {
    ...
    fn test_symbolic_gaussian_correctness() -> Result<()> {
        ...
        let symbolic_result = math.sum(&data, |x: TypedBuilderExpr<f64>| -> TypedBuilderExpr<f64> {
            ...
            let log_normalization = math.constant(0.5) * two_pi.clone().ln() + sigma_param.clone().ln();
        ...
        assert!(error < 1e-10, "Symbolic result differs from manual: {} vs {}", symbolic_value, manual_value);
