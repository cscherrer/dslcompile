...
fn main() {
    ...
    println!("  ✅ Correctness: {}", if (zero_result - expected).abs() < 1e-10 { "PASS" } else { "FAIL" });
    ...
    println!("  ✅ Correctness: {}", if (flexible_result - expected).abs() < 1e-10 { "PASS" } else { "FAIL" });
    ...
    println!("  ✅ Correctness: {}", if (performance_result - expected).abs() < 1e-10 { "PASS" } else { "FAIL" });
    ...
    println!("  ✅ Correctness: {}", if (adaptive_result - expected).abs() < 1e-10 { "PASS" } else { "FAIL" });
    ...
    let mut zero_optimizer = SymbolicOptimizer::with_config(OptimizationConfig::zero_overhead()).unwrap();
