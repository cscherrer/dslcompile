...
fn demo_main() -> Result<()> {
    ...
    for &x_val in &test_values {
        ...
        println!(
            "   x={}: {:.1} (orig: {}ns, opt: {}ns, speedup: {:.1}x)",
            x_val,
            original_result,
            original_time.as_nanos(),
    ...
    println!(
        "   Optimization time: {:.2}μs",
        complex_optimization_time.as_micros()
    );
    ...
    for &(x_val, y_val) in &test_pairs {
        ...
        println!(
            "   x={}, y={}: {:.1} (orig: {}ns, opt: {}ns, speedup: {:.1}x)",
            x_val,
            y_val,
            original_result,
            original_time.as_nanos(),
    ...
    println!(
        "   Compiled execution: {avg_compiled_time:.1}ns/eval ({COMPILED_ITERATIONS} iterations)"
    );
    println!("   vs Optimized eval: {avg_optimized_time:.1}ns/eval");
    println!("   Compiled speedup: {compiled_speedup:.1}x");

    println!("\n✅ Code Generation Verification:");
    println!("   Direct evaluation: {direct_result:.6}");
    println!("   Compiled function: {compiled_result:.6}");
    println!(
        "   Match: {}",
        (compiled_result - direct_result).abs() < 1e-10
    );
    println!(
        "   Correctness over {} iterations: {}",
        COMPILED_ITERATIONS,
    ...
    println!(
        "• Compilation is a one-time cost ({:.2}ms) amortized over many evaluations",
        (codegen_time + compilation_time).as_millis()
    );
...
static BENCHMARK_DATA: std::sync::LazyLock<(
    ASTRepr<f64>,
    ASTRepr<f64>,
    ASTRepr<f64>,
    ASTRepr<f64>,
