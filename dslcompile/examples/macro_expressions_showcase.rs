...
fn basic_arithmetic_demo() {
    ...
    println!("✅ Three-way product: 2 * 3 * 4 = {}", product3(2.0, 3.0, 4.0));
    
    // Complex arithmetic
    let complex = expr!(|a: f64, b: f64, c: f64, d: f64| (a + b) * (c - d));
    println!("✅ Complex: (2 + 3) * (7 - 4) = {}", complex(2.0, 3.0, 7.0, 4.0));
...
fn mathematical_functions_demo() {
    ...
    let compound_interest = expr!(|principal: f64, rate: f64, time: f64| 
        principal * exp(rate * time)
    );
    println!("✅ Compound interest ($1000, 5%, 2 years): ${:.2}", 
             compound_interest(1000.0, 0.05, 2.0));
    
    // Power functions
    let power_law = expr!(|x: f64, a: f64, b: f64| a * pow(x, b));
    println!("✅ Power law (2 * x^3 at x=3): {}", power_law(3.0, 2.0, 3.0));
...
fn mixed_types_demo() {
    ...
    let weighted_sum = expr!(|arr: &[f64], idx1: usize, idx2: usize, factor: f64|
        (arr[idx1] + arr[idx2]) * factor
    );
    println!("✅ Weighted sum: ({} + {}) * {} = {}", 
             weights[1], weights[3], 2.0, 
    ...
    println!("✅ String metric: {} chars * 1.5 = {}", 
             text.len(), string_metric(text.len(), 1.5));
    
    // Boolean to numeric conversion
    let conditional_value = expr!(|condition: bool, true_val: f64, false_val: f64|
        if condition { true_val } else { false_val }
    );
    println!("✅ Conditional: true ? 10.0 : 5.0 = {}", 
...
fn neural_network_demo() {
    ...
    let neuron = expr!(|weights: &[f64], inputs: &[f64], bias: f64|
...
fn financial_calculations_demo() {
    ...
    let present_value = expr!(|future_value: f64, rate: f64, periods: f64|
        future_value / pow(1.0 + rate, periods)
    );
    println!("✅ Present value of $1000 in 5 years at 6%: ${:.2}", 
             present_value(1000.0, 0.06, 5.0));
    
    // Black-Scholes option pricing (simplified)
    let black_scholes_call = expr!(|s: f64, k: f64, r: f64, t: f64, sigma: f64|
        s * 0.5 - k * exp(-r * t) * 0.5  // Simplified approximation
    );
    println!("✅ Option value (simplified): ${:.2}", 
             black_scholes_call(100.0, 95.0, 0.05, 0.25, 0.2));
    
    // Portfolio return calculation
    let portfolio_return = expr!(|weights: &[f64], returns: &[f64], risk_free: f64|
        weights[0] * returns[0] + weights[1] * returns[1] + risk_free
    );
    let weights = [0.6, 0.4];
    let returns = [0.08, 0.12];
    println!("✅ Portfolio return: {:.1}%", 
...
fn physics_demo() {
    ...
    println!("✅ Kinetic energy (10kg at 5m/s): {} J", kinetic_energy(10.0, 5.0));
    
    // Gravitational force
    let gravity = expr!(|m1: f64, m2: f64, r: f64, g: f64| g * m1 * m2 / (r * r));
    let g_constant = 6.67430e-11;
    println!("✅ Gravitational force: {:.2e} N", 
    ...
    let projectile_x = expr!(|v0: f64, angle: f64, time: f64|
        v0 * cos(angle) * time
    );
    println!("✅ Projectile x-position: {:.2} m", 
             projectile_x(20.0, 0.785, 2.0)); // 45 degrees in radians
