...
use dslcompile::{backends::RustCompiler, prelude::*};
...
fn main() -> Result<()> {
    ...
    match compiler.compile_and_load(&poly_code, "polynomial_func") {
        ...
        Err(e) => {
            println!(
                "⚠️ Compilation failed (this is expected in some environments): {}",
                e
            );
    ...
    println!(
        "- StaticCompiler generates inline code that can be embedded directly in your program"
    );
