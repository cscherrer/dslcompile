...
fn main() {
    ...
    println!(
        "Zero Overhead Direct: {:.2}ns per operation → {result} ({:.1}x overhead)",
        direct_ns,
        direct_ns / native_ns
    );
    ...
    println!(
        "AST Interpretation:   {:.2}ns per operation → {result} ({:.1}x overhead)",
        ast_ns,
        ast_ns / native_ns
    );
    ...
    println!(
        "Zero Overhead Direct: {:.2}ns per operation → {result} ({:.1}x overhead)",
        direct_complex_ns,
        direct_complex_ns / native_complex_ns
    );
    ...
    println!(
        "AST Interpretation:   {:.2}ns per operation → {result} ({:.1}x overhead)",
        ast_complex_ns,
        ast_complex_ns / native_complex_ns
    );
    ...
    println!(
        "Optimized AST:        {:.2}ns per operation → {result} ({:.1}x overhead)",
        optimized_ns,
        optimized_ns / native_complex_ns
    );
    ...
    println!(
        "Unoptimized AST:      {:.2}ns per operation → {result} ({:.1}x overhead)",
        const_ast_ns,
        const_ast_ns / native_const_ns
    );
    ...
    println!(
        "Folded AST:           {:.2}ns per operation → {result} ({:.1}x overhead)",
        folded_ns,
        folded_ns / native_const_ns
    );
    ...
    for (name, config) in strategies {
        ...
        println!(
            "{:<17} {:.2}ns per operation → {result} ({:.1}x vs native)",
            format!("{}:", name),
            strategy_ns,
            strategy_ns / native_complex_ns
        );
    }
    ...
    println!("--------------------------|-------------|----------|----------");
    println!(
        "Simple Add (x + y)        | {:>7.2}     | {:>6.2}   | {:>6.1}x",
        native_ns,
        direct_ns,
        direct_ns / native_ns
    );
    println!(
        "Complex (x²+2xy+y²)       | {:>7.2}     | {:>6.2}   | {:>6.1}x",
        native_complex_ns,
        direct_complex_ns,
        direct_complex_ns / native_complex_ns
    );
    println!(
        "Transcendental (sin+cos)  | {:>7.2}     | {:>6.2}   | {:>6.1}x",
        native_trig_ns,
        trig_ast_ns,
        trig_ast_ns / native_trig_ns
    );
    println!(
        "Constant Folding          | {:>7.2}     | {:>6.2}   | {:>6.1}x",
        native_const_ns,
        folded_ns,
        folded_ns / native_const_ns
    );
    ...
    println!(
        "• Zero-overhead direct operations: ~{:.1}x native performance",
        direct_ns / native_ns
    );
    println!(
        "• AST interpretation overhead: ~{:.1}x for simple ops, ~{:.1}x for complex",
        ast_ns / native_ns,
        ast_complex_ns / native_complex_ns
    );
    println!(
        "• Constant folding achieves near-native performance: ~{:.1}x",
        folded_ns / native_const_ns
    );
    println!(
        "• Transcendental functions have consistent overhead: ~{:.1}x",
        trig_ast_ns / native_trig_ns
    );
