...
fn main() -> Result<()> {
    ...
    let sum_expr = ctx.sum(&data, |x_i| {
        &a * &x_i + &b * &x_i
    });
    
    println!("✅ Created expression: Σ(a * x_i + b * x_i) for x_i in {:?}", data);
    println!("   Expected optimization: Σ(a * x_i + b * x_i) → Σ(a * x_i) + Σ(b * x_i) → a * Σ(x_i) + b * Σ(x_i) → (a + b) * Σ(x_i)");
    ...
    #[cfg(feature = "optimization")]
    {
        ...
        match optimizer.optimize(&original_ast) {
            Ok(optimized_ast) => {
                ...
                println!("   Difference: {:.2e}", (original_result - optimized_result).abs());
                ...
                if optimized_ops < original_ops {
                    ...
                    println!("   Reduced operations from {} to {}", original_ops, optimized_ops);
    ...
    #[cfg(not(feature = "optimization"))]
    {
        ...
        println!("   Run with: cargo run --features optimization --example sum_splitting_test_demo");
    }
    ...
    // = a * Σ(x_i) + b * n + c * Σ(x_i²)
    let complex_sum = ctx.sum(&data, |x_i| {
        &a * &x_i + &b + &c * (&x_i * &x_i)
    });
