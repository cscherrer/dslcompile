...
use dslcompile::compile_time::heterogeneous::{
    HeteroContext, HeteroInputs, hetero_add, HeteroExpr
};
...
fn demonstrate_evolution() {
    ...
    println!("Final HeteroContext (zero-overhead): {:?} ({:.2} ns/op)", final_duration, final_duration.as_nanos() as f64 / ITERATIONS as f64);
    
    // Calculate improvement
    let improvement = old_duration.as_nanos() as f64 / final_duration.as_nanos() as f64;
    
    println!("\n📈 Performance Achievement:");
    println!("🚀 Final vs Old Context: {:.2}x faster", improvement);
...
fn demonstrate_scaling() {
    ...
    println!("Final HeteroContext (O(1) array access): {:?} ({:.2} ns/op)", final_duration, final_duration.as_nanos() as f64 / ITERATIONS as f64);
    
    let final_vs_old_scaling = old_duration.as_nanos() as f64 / final_duration.as_nanos() as f64;
    
    println!("\n📊 Scaling Analysis:");
    println!("🚀 Final vs Old Context (8 vars): {:.2}x faster", final_vs_old_scaling);
    ...
    println!("  • 🚀 Native type support: f64, Vec<f64>, usize, custom types - no conversion overhead");
