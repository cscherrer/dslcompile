...
fn main() -> Result<()> {
    ...
    let test_expr = ctx.new_scope(|scope| {
        ...
        let (sum_expr, _) = scope.sum(vec![1.0, 2.0], |x| {
            // x is StaticBoundVar<f64, 0, 0>
    ...
    println!(
        "   Result: {} {}",
        direct_result,
        if (direct_result - 2.0).abs() < 1e-10 {
            "✅"
        } else {
            "❌"
        }
    ...
    println!("\n2. STATICCONTEXT -> ASTREPR -> AST EVALUATION:");
    // Using type-safe HList evaluation (preferred over deprecated eval_with_vars)
    let ast = test_expr.to_ast();
    let ast_result = ast.eval_hlist(&frunk::hlist![0.5]);
    println!(
        "   Result: {} {}",
        ast_result,
        if (ast_result - 2.0).abs() < 1e-10 {
            "✅"
        } else {
            "❌"
        }
    ...
    match test_codegen_evaluation(&test_expr) {
        Ok(codegen_result) => {
            println!(
                "   Result: {} {}",
                codegen_result,
                if (codegen_result - 2.0).abs() < 1e-10 {
                    "✅"
                } else {
                    "❌"
                }
    ...
    println!(
        "   Result: {} {}",
        no_opt_result,
        if (no_opt_result - 2.0).abs() < 1e-10 {
            "✅"
        } else {
            "❌"
        }
