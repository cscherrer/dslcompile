...
fn main() -> Result<()> {
    ...
    println!(
        ...
        result.original_expr.count_operations()
    );
    println!(
        "   Operations in simplified: {}",
        result.simplified_expr.count_operations()
    );

    if let Some(closed_form) = &result.closed_form {
        println!(
            "   Operations in closed form: {}",
            closed_form.count_operations()
        );
    }
    ...
    println!(
        "     (y - β₀ - β₁*x)² = ({y_sample} - {beta0} - {beta1}*{x_sample})² = {eval_result}"
    );
