...
fn main() -> Result<()> {
    ...
    println!("✅ f(μ=0, σ=1, x=1) = {:.6}", single_result);
    ...
    println!("✅ L(μ=0, σ=1, data) = {:.6}", likelihood_result);
    
    // Verify by computing manually
    let manual_sum: f64 = data.iter()
        .map(|&x| {
            let expected = -0.5 * (2.0 * std::f64::consts::PI).ln() - 0.0 - 0.5 * (x - 0.0).powi(2);
            expected
        })
        .sum();
    println!("   Manual computation: {:.6} ✓", manual_sum);
    ...
    #[cfg(feature = "optimization")]
    {
        ...
        match optimize_simple_sum_splitting(&ast) {
            Ok(optimized) => {
                ...
                let large_data: Vec<f64> = (0..10_000).map(|i| (i as f64) * 0.001).collect();
                ...
                println!("   Result: {:.6}", original_result);
                println!("   Time: {:.2?}", original_time);
                ...
                println!("   Result: {:.6}", optimized_result);
                println!("   Time: {:.2?}", optimized_time);
                ...
                if optimized_time < original_time {
                    ...
                    let slowdown = optimized_time.as_nanos() as f64 / original_time.as_nanos() as f64;
    ...
    println!("✅ Log prior p(μ=0): {:.6}", test_prior);
    println!("✅ Log likelihood: {:.6}", likelihood_result);
    println!("✅ Log posterior ∝ {:.6}", test_posterior);
