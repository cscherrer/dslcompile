...
fn test_lambda_conversion_bug_reproduction() {
    ...
    assert!((original_result - expected).abs() < 1e-10, "Original should match expected: {} vs {}", original_result, expected);
    ...
    assert!((optimized_result - expected).abs() < 1e-10, 
        ...
        optimized_result, expected, (optimized_result - expected).abs());
}
...
fn test_multiple_collections_lambda_bug() {
    ...
    assert!(compound_result.is_ok(), "Compound round-trip should succeed: {:?}", compound_result.err());
    ...
    assert!((optimized_compound - expected_compound).abs() < 1e-10, 
        "Optimized compound should match expected: {} vs {} (diff: {})", 
        optimized_compound, expected_compound, (optimized_compound - expected_compound).abs());
}