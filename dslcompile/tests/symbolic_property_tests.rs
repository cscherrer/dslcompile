...
use dslcompile::{
    ast::{
        ast_repr::ASTRepr,
        ast_utils::{collect_variable_indices, expression_depth, count_nodes},
        advanced::{normalize, denormalize, is_canonical},
        expressions_equal_default,
    },
    symbolic::{
        symbolic::{CompilationStrategy, CompilationApproach, SymbolicOptimizer},
        power_utils::{PowerOptConfig, try_convert_to_integer, generate_integer_power_string},
    },
...
fn symbolic_expr_strategy() -> BoxedStrategy<ASTRepr<f64>> {
    ...
    let binary_strategy = (leaf.clone(), leaf.clone()).prop_flat_map(|(left, right)| {
        prop_oneof![
            ...
            Just(ASTRepr::Sub(Box::new(left.clone()), Box::new(right.clone()))),
...
fn algebraic_expr_strategy() -> BoxedStrategy<ASTRepr<f64>> {
    ...
    prop_oneof![
        // x + 0 -> x
        x.clone().prop_map(|x| ASTRepr::add_from_array([x, ASTRepr::Constant(0.0)])),
        // x * 1 -> x  
        x.clone().prop_map(|x| ASTRepr::mul_from_array([x, ASTRepr::Constant(1.0)])),
        // x * 0 -> 0
        x.clone().prop_map(|x| ASTRepr::mul_from_array([x, ASTRepr::Constant(0.0)])),
        // x - x -> 0
        x.clone().prop_map(|x| ASTRepr::Sub(Box::new(x.clone()), Box::new(x))),
        // x^1 -> x
        x.clone().prop_map(|x| ASTRepr::Pow(Box::new(x), Box::new(ASTRepr::Constant(1.0)))),
        // Constant arithmetic
        constants.clone().prop_flat_map(move |a| {
            constants.clone().prop_map(move |b| {
                ASTRepr::add_from_array([a.clone(), b])
            })
        }),
    ].boxed()
}
...
fn contains_sub_div(expr: &ASTRepr<f64>) -> bool {
    match expr {
        ...
        ASTRepr::Ln(inner) | ASTRepr::Exp(inner) | ASTRepr::Sqrt(inner) => contains_sub_div(inner),
        _ => false,
    }
}
...
mod symbolic_unit_tests {
    ...
    fn test_constant_folding() {
        // Test 2 + 3 = 5
        let const_add: ASTRepr<f64> = ASTRepr::add_from_array([
    ...
    fn test_compilation_strategy_creation() {
        ...
        match adaptive {
            CompilationStrategy::Adaptive { call_threshold, complexity_threshold } => {
    ...
    fn test_symbolic_optimizer_basic() {
        ...
        let simple_expr: ASTRepr<f64> = ASTRepr::add_from_array([
            ASTRepr::Variable(0),
