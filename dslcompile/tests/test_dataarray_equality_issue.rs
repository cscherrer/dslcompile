...
use dslcompile::ast::{ast_repr::{ASTRepr, Collection}, multiset::MultiSet};
...
fn test_dataarray_equality_issue() {
    ...
    if multiset.distinct_len() == 1 {
        ...
        println!("✅ No bug: Two Sum expressions with different data are correctly treated as distinct.");
    }
    ...
    if let (ASTRepr::Sum(coll1), ASTRepr::Sum(coll2)) = (&ast1, &ast2) {
        ...
        if let (Collection::Map { lambda: lambda1, collection: coll_inner1 }, 
                Collection::Map { lambda: lambda2, collection: coll_inner2 }) = 
    ...
    assert_ne!(ast1, ast2, "Two Sum expressions with different data should NOT be equal");
    assert_eq!(multiset.distinct_len(), 2, "MultiSet should contain 2 distinct elements");
}