...
use dslcompile::{
    ...
    interval_domain::{Endpoint, IntervalDomain},
    prelude::*,
};
...
fn interval_domain_strategy() -> BoxedStrategy<IntervalDomain<f64>> {
    prop_oneof![
        ...
        (endpoint_strategy(), endpoint_strategy())
            .prop_map(|(lower, upper)| { IntervalDomain::new_interval(lower, upper) }),
    ]
    .boxed()
}
    ...
    prop_oneof![
        ...
        (-50.0..0.0).prop_flat_map(|lower| {
            (0.0..50.0).prop_map(move |upper| {
                IntervalDomain::new_interval(Endpoint::Closed(lower), Endpoint::Closed(upper))
            })
        }),
    ]
    .boxed()
