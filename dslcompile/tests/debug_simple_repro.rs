...
fn debug_simple_repro() {
    ...
    println!("compound eval: {} (expected: {})", compound_eval, test_x * 10.0);
        ...
        for (i, (sum_expr, count)) in terms.iter().enumerate() {
            ...
            if let dslcompile::ast::ast_repr::ASTRepr::Sum(collection) = sum_expr {
                match collection.as_ref() {
                    dslcompile::ast::ast_repr::Collection::Map { lambda, collection: inner } => {
    ...
    println!("optimized eval: {} (expected: {})", optimized_eval, test_x * 10.0);
    
    assert!((optimized_eval - test_x * 10.0).abs() < 1e-10, 
        "Optimization should preserve semantics: {} vs {}", 
        optimized_eval, test_x * 10.0);
}