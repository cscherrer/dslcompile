...
use dslcompile::ast::ast_utils::visitors::{OperationCountVisitor, SummationCountVisitor};
use frunk::hlist;
...
fn test_simple_optimization_should_reduce() {
    ...
    assert!((original_result - optimized_result).abs() < 1e-10,
            "Optimization changed semantics: {} vs {}", original_result, optimized_result);
}
...
fn test_normal_log_density_optimization() {
    ...
    assert!(optimized_ops <= original_ops + 2, // Allow small increase due to canonicalization
...
fn test_iid_summation_optimization() {
    ...
    assert_eq!(original_sums, optimized_sums, "Summation count should be preserved");
