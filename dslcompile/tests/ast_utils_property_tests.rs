...
use dslcompile::ast::{
    ...
    ast_repr::ASTRepr,
    ast_utils::{
        collect_variable_indices, count_nodes, count_operations_visitor, expression_depth,
        expressions_equal_default, extract_constant, extract_variable_index,
        generate_variable_names, is_constant, is_variable, summation_aware_cost_visitor,
...
fn simple_expr_strategy() -> BoxedStrategy<ASTRepr<f64>> {
    ...
    let simple_ops = leaf.clone().prop_flat_map(move |left| {
        leaf.clone()
            .prop_map(move |right| ASTRepr::add_from_array([left.clone(), right]))
    });
...
fn complex_expr_strategy() -> BoxedStrategy<ASTRepr<f64>> {
    ...
    let unary = leaf.clone().prop_map(|inner| ASTRepr::Sin(Box::new(inner)));
...
mod ast_utils_unit_tests {
    ...
    fn test_operation_counting() {
        ...
        let complex_expr: ASTRepr<f64> = ASTRepr::Sin(Box::new(ASTRepr::add_from_array([
            ASTRepr::Variable(0),
    ...
    fn test_transform_identity() {
        let expr: ASTRepr<f64> =
            ASTRepr::add_from_array([ASTRepr::Variable(0), ASTRepr::Constant(1.0)]);
