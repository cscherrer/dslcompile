...
fn debug_sum_ast_equality_detailed() {
    ...
    if let ASTRepr::Sum(coll1) = &ast1 {
        if let Collection::Map { lambda: lambda1, collection: inner1 } = coll1.as_ref() {
    ...
    if let ASTRepr::Sum(coll2) = &ast2 {
        if let Collection::Map { lambda: lambda2, collection: inner2 } = coll2.as_ref() {
    ...
    if let ASTRepr::Add(terms) = &compound_ast {
        ...
        for (i, (term, count)) in terms.iter().enumerate() {
            ...
            if let ASTRepr::Sum(coll) = term {
                if let Collection::Map { lambda: _, collection: inner } = coll.as_ref() {
    ...
    println!("Manual MultiSet distinct length: {}", manual_multiset.distinct_len());
    
    assert_eq!(manual_multiset.distinct_len(), 2, 
