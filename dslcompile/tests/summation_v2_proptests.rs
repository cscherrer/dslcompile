...
fn test_constant_summation(k: f64, start: i64, end: i64) -> std::result::Result<(), TestCaseError> {
    ...
    let mut processor = SummationProcessor::new()
        .map_err(|e| TestCaseError::Fail(format!("Failed to create processor: {e}").into()))?;
    ...
    let optimized_value = result
        .evaluate(&[])
        .map_err(|e| TestCaseError::Fail(format!("Failed to evaluate: {e}").into()))?;
    ...
    match &result.pattern {
        SummationPattern::Constant { value } => {
            ...
            prop_assert!(
                (value - 1.0).abs() < 1e-12,
                "Constant pattern should have value 1.0 after factor extraction, got {}",
                value
            );
        }
    ...
    prop_assert!(
        !result.extracted_factors.is_empty(),
        "Should extract constant factor"
    );
    let total_factor: f64 = result.extracted_factors.iter().product();
    prop_assert!(
        (total_factor - k).abs() < 1e-12,
        "Should extract factor {}, got {}",
        k,
        total_factor
    );
    ...
    // Check correctness
    prop_assert!(
        (optimized_value - expected).abs() < 1e-10,
        "Constant sum: expected {}, got {}",
        expected,
        optimized_value
    );
...
fn test_linear_summation(
    ...
    match &result.pattern {
        SummationPattern::Linear {
            ...
            prop_assert!(
                (coefficient - coeff).abs() < 1e-12,
                "Linear coefficient: expected {}, got {}",
                coeff,
                coefficient
            );
            prop_assert!(
                (c - constant).abs() < 1e-12,
                "Linear constant: expected {}, got {}",
                constant,
                c
            );
        }
        _ => prop_assert!(
            false,
            "Should recognize linear pattern, got {:?}",
            result.pattern
        ),
    }
    ...
    prop_assert!(
        (optimized_value - brute_force_value).abs() < 1e-10,
        "Linear sum: expected {}, got {}",
        brute_force_value,
        optimized_value
    );
...
fn test_factor_extraction(
    ...
    prop_assert!(
        (total_factor - expected_total_factor).abs() < 1e-12,
        "Total factor: expected {}, got {}",
        expected_total_factor,
        total_factor
    );
    ...
    prop_assert!(
        (optimized_value - brute_force_value).abs() < 1e-10,
        "Factor extraction: expected {}, got {}",
        brute_force_value,
        optimized_value
    );
...
fn test_geometric_series(
    ...
    prop_assume!((1..=20).contains(&max_terms)); // Keep geometric series reasonable
    ...
    prop_assert!(
        (optimized_value - brute_force_value).abs() < 1e-8,
        "Geometric series: expected {}, got {}",
        brute_force_value,
        optimized_value
    );
...
fn test_power_series(
    ...
    prop_assume!((0.0..=4.0).contains(&exponent)); // Reasonable exponents
    ...
    if exponent == exponent.round() {
        match &result.pattern {
            SummationPattern::Power { exponent: e } => {
                prop_assert!(
                    (e - exponent).abs() < 1e-12,
                    "Power exponent: expected {}, got {}",
                    exponent,
                    e
                );
            }
            _ => {
                ...
                println!(
                    "Power pattern not recognized for exponent {}, got {:?}",
                    exponent, result.pattern
                );
            }
        }
    }
...
fn test_distributivity(
    ...
    let mut processor2 = SummationProcessor::new()
        .map_err(|e| TestCaseError::Fail(format!("Failed to create processor2: {e}").into()))?;
    let separate_result = processor2
        .sum(range.clone(), |i| {
            let math = ExpressionBuilder::new();
    ...
    let combined_value = combined_result
        .evaluate(&[])
        .map_err(|e| TestCaseError::Fail(format!("Failed to evaluate combined: {e}").into()))?;
    let separate_value = separate_result
        .evaluate(&[])
        .map_err(|e| TestCaseError::Fail(format!("Failed to evaluate separate: {e}").into()))?
        * k;

    // Both should give the same result
    prop_assert!(
        (combined_value - separate_value).abs() < 1e-10,
        "Distributivity: combined {}, separate {}",
        combined_value,
        separate_value
    );
    ...
    match (&combined_result.pattern, &separate_result.pattern) {
        ...
        _ => {
            ...
            println!(
                "Pattern recognition varied: combined {:?}, separate {:?}",
...
mod tests {
    ...
    fn test_manual_summation_cases() -> Result<()> {
        ...
        // Σ(i=1 to 10) i = 55
        test_linear_summation(1.0, 0.0, 1, 10).map_err(|_| {
            dslcompile::error::DSLCompileError::Generic("proptest error".to_string())
        })?;

        // Σ(i=1 to 5) 2*i = 2*15 = 30
        test_factor_extraction(2.0, 1.0, 1, 5).map_err(|_| {
            dslcompile::error::DSLCompileError::Generic("proptest error".to_string())
        })?;

        // Σ(i=1 to 4) i² = 1+4+9+16 = 30
        test_power_series(2.0, 1, 4).map_err(|_| {
            dslcompile::error::DSLCompileError::Generic("proptest error".to_string())
        })?;
