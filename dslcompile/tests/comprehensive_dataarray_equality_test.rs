...
fn test_comprehensive_dataarray_equality() {
    ...
    test_distinct_sums(
        ...
        &sum1,
        &sum2,
        "Scenario 1: Same lambda, different data",
    );
    ...
    test_distinct_sums(
        ...
        &sum3,
        &sum4,
        "Scenario 2: Different lambdas, same data",
    );
    ...
    test_compound_expression(
        &ctx,
        &compound,
        "Scenario 4: Addition of sums with different data",
    );
    ...
    test_distinct_sums(
        ...
        &complex_sum1,
        &complex_sum2,
        "Scenario 5: Complex lambda, different data",
    );
...
fn test_identical_sums<const S: usize>(
    ...
    assert_eq!(
        multiset.distinct_len(),
        1,
        "{}: MultiSet should contain 1 element (merged)",
        scenario_name
    );
}
...
fn test_compound_expression<const S: usize>(
    ...
    if let ASTRepr::Add(terms) = &compound_ast {
        println!(
            "Compound expression has {} distinct terms",
            terms.distinct_len()
        );
...
fn test_dataarray_specific_edge_cases() {
    ...
    println!(
        "Different order arrays equal: {}",
        ast_ordered1 == ast_ordered2
    );
    assert_ne!(
        ast_ordered1, ast_ordered2,
        "Different order arrays should NOT be equal"
    );
    ...
    assert_ne!(
        ast_dup1, ast_dup2,
        "Arrays with different duplicate patterns should NOT be equal"
    );
