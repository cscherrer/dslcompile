...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    // h(x) = sin(x)
    let sine =
        MathFunction::from_lambda("sin", |builder| builder.lambda(|x: LambdaVar<f64>| x.sin()));
    ...
    println!(
        "  Polynomial: 3x³ - 2x² + x - 1 = {}",
        polynomial.eval(hlist![test_val])
    );
    println!(
        "  Trigonometric: sin(x)cos(x) + e^x/(x+1) = {}",
        trigonometric.eval(hlist![test_val])
    );
    println!();
