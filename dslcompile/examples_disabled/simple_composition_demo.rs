...
fn main() -> Result<()> {
    ...
    #[cfg(feature = "optimization")]
    {
        ...
        let simple_reduction =
            if simple_ast.count_operations() > optimized_simple.count_operations() {
                format!(
                    "{} operations eliminated!",
                    simple_ast.count_operations() - optimized_simple.count_operations()
                )
            } else {
        ...
        let complex_reduction =
            if complex_ast.count_operations() > optimized_complex.count_operations() {
                format!(
                    "{} operations eliminated!",
                    complex_ast.count_operations() - optimized_complex.count_operations()
                )
            } else {
                "No reduction found".to_string()
            };
    ...
    #[cfg(not(feature = "optimization"))]
    {
        ...
        println!(
            "   Run with: cargo run --features optimization --example simple_composition_demo"
        );
    }
...
fn compute_depth<T>(ast: &ASTRepr<T>) -> usize {
    match ast {
        ...
        ASTRepr::Add(operands) => 1 + operands.iter().map(compute_depth).max().unwrap_or(0),
        ASTRepr::Sub(l, r) | ASTRepr::Div(l, r) | ASTRepr::Pow(l, r) => {
            1 + std::cmp::max(compute_depth(l), compute_depth(r))
        }
        ASTRepr::Mul(operands) => 1 + operands.iter().map(compute_depth).max().unwrap_or(0),
