...
fn main() -> Result<()> {
    ...
    let large_new_cost_10k =
        SummationAwareCostVisitor::compute_cost_with_domain_size(&large_sum_ast, 10_000);
    let large_new_cost_100k =
        SummationAwareCostVisitor::compute_cost_with_domain_size(&large_sum_ast, 100_000);
    let large_new_cost_1m =
    ...
    let nested_new_cost_small =
        SummationAwareCostVisitor::compute_cost_with_domain_size(&nested_sum_ast, 10); // 10x10
    let nested_new_cost_large =
    ...
    let unfactored_new =
        SummationAwareCostVisitor::compute_cost_with_domain_size(&unfactored_ast, 10_000);
    let factored_new =
    ...
    println!(
        ...
        factored_old,
        if unfactored_old > 0 {
            100 * (unfactored_old - factored_old) / unfactored_old
        } else {
            0
        }
    );
    println!(
        ...
        factored_new,
        if unfactored_new > 0 {
            100 * (unfactored_new - factored_new) / unfactored_new
        } else {
            0
        }
    );
