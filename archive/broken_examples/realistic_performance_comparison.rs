...
fn main() -> Result<()> {
    ...
    println!(
        "Zero Overhead Direct: {:.2}ns per operation (sum: {:.2}) [{:.1}x overhead]",
        direct_ns,
        result,
        direct_ns / native_ns
    );
    ...
    println!(
        "AST Interpretation:   {:.2}ns per operation (sum: {:.2}) [{:.1}x overhead]",
        ast_ns,
        result,
        ast_ns / native_ns
    );
    ...
    println!(
        "Optimized AST:        {:.2}ns per operation (sum: {:.2}) [{:.1}x overhead]",
        optimized_ns,
        result,
    ...
    for (name, config) in strategies {
        ...
        println!(
            "{:<17} {:.2}ns per operation (sum: {:.2}) [{:.1}x vs native]",
            format!("{}:", name),
            strategy_ns,
            result,
    ...
    let start = Instant::now();
    result = 0.0;
    for &idx in &random_indices[..100_000] {
        // Smaller sample for random access
    ...
    println!(
        "Random Access:        {:.2}ns per operation [{:.1}x slower]",
        random_ns,
    ...
    println!("--------------------------|-------------|--------------|----------|----------");
    println!(
        "Simple Add (x + y)        | {:>7.2}     | {:>8.2}     | {:>6.2}   | {:>6.1}x",
        native_ns,
        direct_ns,
        ast_ns,
        ast_ns / native_ns
    ...
    println!(
        "Complex (x²+2xy+y²)       | {:>7.2}     | {:>8.2}     | {:>6.2}   | {:>6.1}x",
        native_complex_ns,
        direct_complex_ns,
        ast_complex_ns,
        ast_complex_ns / native_complex_ns
    ...
    println!(
        "Transcendental (sin+cos)  | {:>7.2}     | {:>8}       | {:>6.2}   | {:>6.1}x",
        native_trig_ns,
        "N/A",
        trig_ast_ns,
        trig_ast_ns / native_trig_ns
    ...
    println!(
        "• Zero-overhead direct: {:.1}x native performance",
        direct_ns / native_ns
    );
    println!(
        "• AST interpretation: {:.1}x overhead for simple, {:.1}x for complex",
        ast_ns / native_ns,
        ast_complex_ns / native_complex_ns
    );
    println!(
        "• Transcendental overhead: {:.1}x (mostly function call cost)",
        trig_ast_ns / native_trig_ns
    );
    println!(
        "• Memory access patterns matter: {:.1}x difference",
        random_ns / sequential_ns
    );
