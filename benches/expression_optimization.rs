...
use mathjit::final_tagless::{JITEval, JITMathExpr, DirectEval};
use mathjit::symbolic::{SymbolicOptimizer, OptimizationConfig, CompilationStrategy, RustOptLevel};
...
fn create_medium_expression() -> mathjit::final_tagless::JITRepr<f64> {
    ...
    JITEval::add(
        JITEval::add(
            ...
            JITEval::ln(JITEval::exp(JITEval::var("x")))
        ),
        JITEval::mul(
            JITEval::add(JITEval::var("y"), JITEval::constant(0.0)),
            JITEval::constant(1.0)
        )
    )
}
...
fn bench_direct_evaluation(c: &mut Criterion) {
    ...
    group.bench_function("simple", |b| {
        b.iter(|| {
            DirectEval::eval_two_vars(black_box(&simple_expr), black_box(x), black_box(y))
        })
    });
    
    group.bench_function("medium", |b| {
        b.iter(|| {
            DirectEval::eval_two_vars(black_box(&medium_expr), black_box(x), black_box(y))
        })
    });
    
    group.bench_function("complex", |b| {
        b.iter(|| {
            DirectEval::eval_two_vars(black_box(&complex_expr), black_box(x), black_box(y))
        })
    });
...
fn bench_optimization_comparison(c: &mut Criterion) {
    ...
    println!("Basic optimized operations: {}", basic_optimized.count_operations());
    println!("Advanced optimized operations: {}", advanced_optimized.count_operations());
...
fn bench_compilation_strategies(c: &mut Criterion) {
    ...
    let rust_code = rust_optimizer.generate_rust_source(&optimized_expr, "bench_func").unwrap();
    
    let source_path = source_dir.join("bench_func.rs");
    let lib_path = lib_dir.join("libbench_func.so");
    
    // Compile the Rust library
    if rust_optimizer.compile_rust_dylib(&rust_code, &source_path, &lib_path, &RustOptLevel::O2).is_ok() && lib_path.exists() {
        // Load the library and benchmark execution
        if let Ok(lib) = unsafe { Library::new(&lib_path) } {
            if let Ok(func) = unsafe { lib.get::<Symbol<unsafe extern "C" fn(f64, f64) -> f64>>(b"bench_func_two_vars") } {
                group.bench_function("rust_compiled", |b| {
                    b.iter(|| {
                        unsafe { func(black_box(x), black_box(y)) }
                    })
                });
            }
        }
...
fn bench_optimization_tradeoff(c: &mut Criterion) {
    ...
    group.bench_function("optimized_execution", |b| {
        b.iter(|| {
            DirectEval::eval_two_vars(black_box(&optimized), black_box(x), black_box(y))
        })
    });
