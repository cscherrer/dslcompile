...
use mathjit::symbolic::{SymbolicOptimizer, OptimizationConfig};
...
fn bench_optimization_performance(c: &mut Criterion) {
    ...
    group.bench_function("cranelift_jit", |b| {
        b.iter(|| {
            ...
            let jit_func = jit_compiler.compile_two_vars(&advanced_optimized, "x", "y").unwrap();
    ...
    println!("Compilation time: {} μs", jit_func.stats.compilation_time_us);
...
fn bench_rust_generation(c: &mut Criterion) {
    ...
    group.bench_function("rust_code_generation", |b| {
        b.iter(|| {
            optimizer.generate_rust_source(black_box(&optimized), "bench_func").unwrap()
        })
    });
    
    // Show generated code
    let rust_code = optimizer.generate_rust_source(&optimized, "optimized_func").unwrap();
    println!("\n🦀 Generated Rust Code:");
    println!("{rust_code}");
...
fn bench_execution_strategies(c: &mut Criterion) {
    ...
    let compilation_cost_ns = jit_func.stats.compilation_time_us as u128 * 1000;
    ...
    if jit_eval_ns > 0 && direct_eval_ns > jit_eval_ns {
        ...
        println!("Compilation cost: {} μs", jit_func.stats.compilation_time_us);
        println!("Direct eval time: {} ns", direct_eval_ns);
        println!("JIT eval time: {} ns", jit_eval_ns);
        println!("JIT pays off after ~{} calls", breakeven_calls);
    }
